...
import { login as deepseekLogin } from '../provider/deepseek/api';
import { login as claudeLogin } from '../provider/claude/api';
import { login as geminiLogin } from '../provider/gemini/api';
import { login as groqLogin } from '../provider/groq/api';
import { login as mistralLogin } from '../provider/mistral/api';
import { login as huggingchatLogin } from '../provider/huggingchat/api';
import { login as kimiLogin } from '../provider/kimi/api';
import { login as qwenLogin } from '../provider/qwen/api';
import { login as cerebrasLogin } from '../provider/cerebras/api';

// CLI Provider login functions
import {
  login as qwenCLILogin,
  getDeviceCode as qwenCLIGetDeviceCode,
  completeLogin as qwenCLICompleteLogin,
  loginWithBrowser as qwenCLILoginWithBrowser,
} from '../provider/qwen-cli/api';
import {
  login as geminiCLILogin,
  loginWithTerminal as geminiCLILoginWithTerminal,
} from '../provider/gemini-cli/api';
import { login as kiroLogin } from '../provider/kiro/api';
import { login as iflowLogin } from '../provider/iflow/api';
import { loginWithTerminal as codexLoginWithTerminal } from '../provider/codex-cli/api';
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.info(`Requested provider: ${providerId}, params: ${JSON.stringify(req.params)}, body: ${JSON.stringify(req.body)}, method: ${req.method}, url: ${req.originalUrl || req.url}`);

    // Use provider-specific login function
    let result;
    const normalizedProviderId = (providerId as string).toLowerCase();

    switch (normalizedProviderId) {
      case 'deepseek':
        result = await deepseekLogin({
          deepseekMethod: method === 'google' ? 'google' : 'basic',
        });
        break;
      case 'claude':
      ...
      case 'gemini':
      case 'google':
        result = await geminiLogin();
        break;
      case 'groq':
        result = await groqLogin();
        break;
      case 'mistral':
        result = await mistralLogin();
        break;
      case 'huggingchat':
        result = await huggingchatLogin();
        break;
      case 'kimi':
        result = await kimiLogin();
        break;
      case 'qwen':
        result = await qwenLogin();
        break;
      case 'cerebras':
        result = await cerebrasLogin();
        break;
      // CLI Providers
      case 'qwen-cli':
      case 'qwencli':
        result = await qwenCLILoginWithBrowser();
        break;
      case 'gemini-cli':
      case 'geminicli':
        result = await geminiCLILoginWithTerminal();
        break;
      case 'kiro':
        result = await kiroLogin();
        break;
      case 'iflow':
        result = await iflowLogin();
        break;
      case 'codex-cli':
      case 'codexcli':
        result = await codexLoginWithTerminal();
        break;
      default:
        // Generic login for other providers
        let loginUrl = '';
        let partition = `persist:${normalizedProviderId}`;
        let cookieEvent = `${normalizedProviderId}-cookies`;
        let infoEvent = '';

        if (normalizedProviderId === 'claude') {
          loginUrl = 'https://claude.ai/login';
        } else if (normalizedProviderId === 'gemini' || normalizedProviderId === 'google') {
    ...
    res.status(200).json({
      ...
      account: {
        ...
        email: (result as any).email || 'captured@account.com',
