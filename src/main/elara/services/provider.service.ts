  ...
  const tryFetch = async (): Promise<any[]> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.warn(`GitHub fetch failed: ${response.statusText}`);
      }
    } catch (error) {
      logger.warn('Error fetching from GitHub (falling back to local):', error);
    }
    ...
    try {
      ...
      const possiblePaths = [
        path.join(process.cwd(), 'src', 'main', 'elara', 'provider.json'),
...
export const getAllProviders = async (): Promise<Provider[]> => {
  ...
  // Get account counts directly from accounts table to ensure accuracy
  const dbAccountCounts = db
    .prepare('SELECT LOWER(provider_id) as provider_id, COUNT(*) as count FROM accounts GROUP BY LOWER(provider_id)')
    .all() as {
    provider_id: string;
    count: number;
  }[];
  ...
  const countsMap = new Map(
    dbAccountCounts.map((p) => [p.provider_id, p.count]),
  );
