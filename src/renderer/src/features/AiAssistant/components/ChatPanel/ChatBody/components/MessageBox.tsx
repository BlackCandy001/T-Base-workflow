...
import ToolItem from "./ToolActions/ToolItem";
        ...
        (() => {
          ...
          return groups.map((group, index) => {
            ...
            if (group.type === "metadata") {
              ...
              const htmlContent = (DOMPurify as any).sanitize
                ? (DOMPurify as any).sanitize(marked.parse(group.content) as string)
                : (DOMPurify as unknown as any).default.sanitize(marked.parse(group.content) as string);
                  ...
                  <div style={{ paddingLeft: "29px", paddingTop: "4px" }}>
                    {group.segments.map((seg: any, i: number) => {
                      if (seg.type === "code") {
                        ...
                        const htmlContent = (DOMPurify as any).sanitize
                          ? (DOMPurify as any).sanitize(marked.parse(seg.content) as string)
                          : (DOMPurify as unknown as any).default.sanitize(marked.parse(seg.content) as string);
                        return (
                          <div
                            key={i}
