...
import { Search, ChevronRight, X, User, ChevronLeft, LogIn, Loader2 } from "lucide-react";
...
const QuickSwitchDrawer: React.FC<QuickSwitchDrawerProps> = ({
  ...
  const handleLogin = async (providerId: string) => {
    ...
    try {
      ...
      if (result.success) {
        // Refresh accounts for the provider
        setStep("account"); // Triggers re-fetch
      } else {
  ...
  return (
    <div
      style={{
        ...
        marginBottom: "8px",
        height: "400px",
        maxHeight: "50vh", // Don't exceed half the screen
        backgroundColor: "rgba(24, 24, 27, 0.95)", // Solid dark zinc base
        backdropFilter: "blur(16px)", // Glassmorphism
        border: "1px solid rgba(255, 255, 255, 0.1)",
        borderRadius: "12px",
        boxShadow: "0 -8px 24px rgba(0, 0, 0, 0.5)", // Darker shadow for depth
        zIndex: 1000,
        display: "flex",
          ...
          <div style={{ position: "relative", marginBottom: "12px" }}>
            ...
            <input
              ...
              style={{
                ...
                backgroundColor: "rgba(0, 0, 0, 0.3)", // Translucent dark field
        ...
        >
          ...
          >
            {providerAccounts.length > 0 ? (
              ...
              >
                <div style={{ opacity: 0.6 }}>No accounts available for this provider.</div>
                {error && <div style={{ color: "#f44336", fontSize: "11px" }}>{error}</div>}
                <button
                  onClick={() => handleLogin(selectedModel.provider_id)}
                ...
                >
                  ...
                  {isLoggingIn ? "Logging in..." : "Login to Provider"}
                </button>
              </div>
            )}
          </div>
