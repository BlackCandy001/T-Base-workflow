...
import AccountDrawer from "./ChatFooter/components/AccountDrawer";
...
const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  // Listen for showAccountDrawer message from WelcomeUI and other sources
  useEffect(() => {
    const handleAccountDrawerMsg = (event: MessageEvent) => {
      if (event.data && typeof event.data === 'object' && event.data.command === 'showAccountDrawer') {
        setIsAccountDrawerOpen(true);
      }
    };
    window.addEventListener('message', handleAccountDrawerMsg);
    return () => window.removeEventListener('message', handleAccountDrawerMsg);
  }, []);
  ...
  }, [selectedTab, currentConversationId, deleteConversation, setMessages, setIsProcessing, setCurrentConversationId]);
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        (message.command === "clearConversationConfirmed" || message.command === "clearChatConfirmed") &&
  ...
  }, [currentConversationId, selectedTab]);
    ...
    const handler = (event: MessageEvent) => {
      ...
      if (data.command === "conversationResult") {
        if (data.data?.messages) {
          ...
          // Restore Main Model and Account from the last message with metadata
          const lastMsgWithMeta = [...data.data.messages]
            .reverse()
            .find(
              (m: Message) => m.providerId && m.modelId,
            );

          if (lastMsgWithMeta) {
            setCurrentModel({
              providerId: lastMsgWithMeta.providerId!,
              id: lastMsgWithMeta.modelId!,
              name: lastMsgWithMeta.modelId!, // Fallback name
            });
            setCurrentAccount({
              id: lastMsgWithMeta.accountId!,
              email: lastMsgWithMeta.email!,
            });
  ...
  }, [currentConversationId, handleClearConfirmed]);

  // Derive if a model is currently "active" or can be recovered from history
  const hasResolvedModel = useMemo(() => {
    if (currentModel || selectedQuickModel) return true;
    // Check history
    return messages.some((m) => m.role === "assistant" && m.providerId && m.modelId);
  }, [currentModel, selectedQuickModel, messages]);
