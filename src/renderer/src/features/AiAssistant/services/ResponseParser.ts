...
export const parseAIResponse = (content: string): ParsedResponse => {
  ...
  const toolPatterns = [
    ...
    "diagram", // Treat <diagram> as a special tag
  ];
    ...
    if (index !== -1 && match) {
      ...
      if (isClosed) {
        ...
        if (toolName === "code") {
          ...
          // Explicit <diagram> tag
          if (innerContent && innerContent.trim()) {
            try {
              // Strip markdown code block wrappers if present
              const cleanJson = innerContent
                .trim()
                .replace(/^```(?:json)?\s*\n?|\n?```\s*$/g, "");
              ...
              if (diagramData.nodes && diagramData.edges) {
                ...
                result.diagrams.push(diagramData);
              }
            } catch (e) {
        ...
        if (toolName === "task_progress") {
          ...
          // Show a placeholder for the diagram while it's still streaming
          result.contentBlocks.push({
            type: "diagram",
            nodes: [],
            edges: [],
