...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      // console.error("[App] Failed to clear stale state:", _error);
    }
  }, []);
  ...
  const tabs = useMemo<TabInfo[]>(() => [], []); // WebSocket removed, using empty tabs for now
  const handleMessage = useCallback((_data: any) => {}, []);
